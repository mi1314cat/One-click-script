#!/usr/bin/env bash
# UFW 防火墙管理面板 v8.2(旗舰版 + SSH 动态适配 + 安全同步)
# - UI 美化(渐变标题 + 蓝色选项 + 子菜单)
# - 入站严格控制 / 出站全放行(适配 Vultr)
# - 多 SSH 端口支持(永不锁死,禁止关闭 SSH 端口)
# - SSH 端口变更后一键同步规则(先加后删,绝不失联)
# - 端口占用检测(TCP/UDP)
# - 自动检测端口是否已开放
# - 自动检测 Web 服务(Nginx/Caddy)+ 快捷管理
# - 动态加载动画
# - 一键备份 / 恢复规则
# - 清空规则保留 SSH
# - 去除全局 set -e,改为局部防呆,避免意外退出
# - 安装后自动 systemctl enable ufw
...
if ! command -v ufw >/dev/null 2>&1; then
  ...
  if [[ "$ans" =~ ^(Y|y|)$ ]]; then
    if command -v apt >/dev/null 2>&1; then
      ...
      print_error "无法自动安装 UFW,请手动安装后重试"
      exit 1
    fi
    # 开机自启 + 启动服务(用户要求)
    systemctl enable ufw 2>/dev/null || true
    systemctl start ufw 2>/dev/null || true
    print_info "UFW 安装成功,并已设置 systemctl enable ufw"
    log "已自动安装 UFW 并启用开机自启"
  else
...
sync_ssh_ports() {
  ...
  if [[ -n "$session_port" && ! " $new_ports " =~ " $session_port " ]]; then
    print_error "⚠️ 严重警告:您当前的 SSH 连接端口 ($session_port) 不在新的监听端口列表中!"
    print_error "   如果继续同步,您可能会立即断开连接。"
    print_warn "   建议先在 sshd_config 中同时保留新旧端口,重启 sshd,再执行同步。"
    read -r -p "风险极高,是否仍然继续?(y/N): " force || true
    if [[ ! "$force" =~ ^[Yy]$ ]]; then
      print_info "取消同步"
      return 0
    fi
  ...
  print_warn "是否更新防火墙规则?此操作会先添加新端口的规则,再删除旧端口的规则。"
  print_warn "注意:同步过程会保持旧规则直到最后一步删除,您当前的 SSH 连接不会中断。"
  read -r -p "确认同步?(y/N): " confirm || true
  if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
    print_info "取消同步"
    return 0
  fi
  ...
  # 第一步:添加所有新端口的规则(已存在则跳过)
  for port in $new_ports; do
    add_ssh_rule "$port"
  done

  # 第二步:删除旧端口的规则(仅删除带注释且不在新端口列表中的)
  ufw status 2>/dev/null | grep "$SSH_COMMENT" | while IFS= read -r line; do
...
web_service_menu() {
  ...
  case "$webop" in
    1) loading "正在启动 Nginx..."; systemctl start nginx 2>/dev/null && print_info "Nginx 已启动" || print_error "启动失败" ;;
    2) loading "正在停止 Nginx..."; systemctl stop nginx 2>/dev/null && print_info "Nginx 已停止" || print_error "停止失败" ;;
    3) loading "正在重载 Nginx..."; systemctl reload nginx 2>/dev/null && print_info "Nginx 已重载" || print_error "重载失败" ;;
    4) loading "正在启动 Caddy..."; systemctl start caddy 2>/dev/null && print_info "Caddy 已启动" || print_error "启动失败" ;;
    5) loading "正在停止 Caddy..."; systemctl stop caddy 2>/dev/null && print_info "Caddy 已停止" || print_error "停止失败" ;;
    6) loading "正在重载 Caddy..."; systemctl reload caddy 2>/dev/null && print_info "Caddy 已重载" || print_error "重载失败" ;;
    0) ;;
    *) print_error "无效选择" ;;
  esac
}
...
check_port_open() {
  ...
  if ufw status 2>/dev/null | grep -qE "^$port/"; then
    print_info "端口 $port 已在 UFW 中开放"
  else
    print_error "端口 $port 未在 UFW 中开放"
  fi
}
...
show_open_ports() {
  ...
  echo -e "${CYAN}╚══════════════════════════════════════════════╝${RESET}"
  local total
  total="$(ufw status 2>/dev/null | grep -cE '^[0-9]+/tcp|^[0-9]+/udp' || true)"
  print_info "当前 UFW 共 $total 条端口放行规则"
}
...
restore_rules() {
  ...
  ufw reload >/dev/null
  print_warn "规则已从备份恢复,请确认 SSH 连接正常后再关闭本窗口"
  log "从备份恢复规则:$file"
}
...
delete_ufw() {
  ...
  print_info "停止并禁用 UFW 服务..."
  systemctl stop ufw >/dev/null 2>&1 || true
  systemctl disable ufw >/dev/null 2>&1 || true

  echo
  echo -e "${YELLOW}是否卸载 ufw 包?${RESET}"
  echo -e "1) 卸载"
  echo -e "2) 保留"
  ...
  print_info "UFW 已成功卸载/清除(SSH 不受影响)"
  print_info "脚本即将退出"
  exit 0
}
...
while true; do
  ...
  RAW_STATUS="$(ufw status 2>/dev/null | head -n1 | awk '{print $2}')"
