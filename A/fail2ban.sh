...
#  修复1: 全部 =~ 正则改用变量形式, 消除 "unexpected token ')'" 语法错误
#  修复2: 去除 POSIX ERE 不支持的 (?:...) 非捕获组, 改为 (reading|configuring)
#  输出 : 统一 print_info / print_error 彩色风格
#  特性 : 强制探测运行 jail / 双层状态(检测+拦截) / 拦截链路自检
# ============================================================================
set -o pipefail

[[ $EUID -ne 0 ]] && { echo -e " \033[31m[Error]\033[0m 请以 root 身份运行"; exit 1; }
...
print_info()   { echo -e " ${GREEN}[Info]${PLAIN} $*"; }
print_ok()     { echo -e " ${GREEN}[ OK ]${PLAIN} $*"; }
print_warn()   { echo -e " ${YELLOW}[Warn]${PLAIN} $*"; }
print_error()  { echo -e " ${RED}[Error]${PLAIN} $*"; }
print_step()   { echo -e " ${BLUE}[ >> ]${PLAIN} $*"; }
...
INI_SET() { # file section key value
    ...
    if command -v crudini >/dev/null 2>&1; then
        crudini --set "$file" "$section" "$key" "$value"; return $?
    fi
    print_warn "未安装 crudini, 配置修改受限, 请先安装 crudini"
    return 1
}
...
refresh_status() {
    ...
    local raw jail_list j out cur_fail tot_fail cur_ban tot_ban banlist
    ...
    for j in $jail_list; do
        ...
        banlist=$(echo "$banlist" | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | tr '\n' ' ' | xargs)
...
status_table() {
    ...
    for j in $F2B_JAIL_ORDER; do
        printf "%-14s %10s %12s %12s %12s\n" \
            "$j" "${F2B_CUR_FAIL[$j]:-0}" "${F2B_TOT_FAIL[$j]:-0}" \
            "${F2B_CUR_BAN[$j]:-0}" "${F2B_TOT_BAN[$j]:-0}"
    done
}
...
extract_jail_failures() {
    ...
    local re_jail="jail[[:space:]]*['\"]([^'\"]+)['\"]"
    local re_while="while[[:space:]](reading|configuring)[[:space:]]jail[[:space:]]*'([^']+)'"
    local re_common="(sshd|recidive|postfix|dovecot|nginx-http-auth)"

    local line jail_name reason
    while IFS= read -r line; do
        [[ "$line" == *"already banned"* ]] && continue
        jail_name=""
        if [[ "$line" =~ $re_jail ]]; then
            jail_name="${BASH_REMATCH[1]}"
        elif [[ "$line" =~ $re_while ]]; then
            jail_name="${BASH_REMATCH[2]}"
        elif [[ "$line" =~ $re_common ]]; then
            jail_name="${BASH_REMATCH[1]}"
        fi
...
diag_blocking() {
    ...
    if [[ -n "$SSH_LOGFILE" && ! -f "$SSH_LOGFILE" ]]; then
        print_warn "SSH 日志文件不存在, 建议使用 systemd 后端"
    fi

    echo
    print_step "③ 真实写入测试 (封禁测试 IP 192.0.2.1, 数秒后自动解封)"
    local test_ip="192.0.2.1" test_jail
    ...
    echo
    print_info "结论: ① 有失败记录 + ③ 规则写入成功 → 说明只是暂时没攻击/未触发阈值; 否则按提示修复"
}
...
fix_ssh_logpath() {
    ...
    if [[ -f /etc/fail2ban/jail.local ]]; then
        ...
        grep -qE '^[[:space:]]*backend' /etc/fail2ban/jail.local 2>/dev/null || \
...
apply_recommended() {
    ...
    local ba ssh_port f2b_log ssh_log
    ...
    f2b_log="${F2B_LOGFILE:-/var/log/fail2ban.log}"
    ssh_log="${SSH_LOGFILE:-/var/log/auth.log}"
...
select_jail() {
    ...
    local names=($F2B_JAIL_ORDER) input idx i
    echo "可用 jail:"
    ...
    if [[ "$input" =~ ^[0-9]+$ ]]; then
        ...
        if [[ $idx -ge 0 && $idx -lt ${#names[@]} ]]; then
            SELECTED_JAIL="${names[$idx]}"
        else
            print_error "无效序号"; return 1
        fi
    else
        SELECTED_JAIL="$input"
        if [[ " $F2B_JAIL_ORDER " != *" $SELECTED_JAIL "* ]]; then
            print_error "jail '$SELECTED_JAIL' 不存在"; return 1
        fi
    fi
    return 0
}
...
jail_management_menu() {
    while true; do
        ...
        if [[ -z "$all_jails" ]]; then
            print_warn "未找到 jail 定义"; pause; return
        fi
        echo "所有 Jail (序号切换启用状态):"
        local i=1 j status c
        declare -A jmap
...
live_monitor() {
    trap 'echo; print_info "监控结束"; return' INT
    while true; do
        clear_screen
        ...
        if [[ $F2B_JAIL_COUNT -gt 0 ]]; then
            ...
            echo -e "   ${YELLOW}无运行中的 jail${PLAIN}"
        fi
        echo
        echo -e "${BOLD}最近事件:${PLAIN}"
        if [[ -n "$F2B_LOGFILE" && -f "$F2B_LOGFILE" ]]; then
            tail -6 "$F2B_LOGFILE" 2>/dev/null
        else
            journalctl -u fail2ban -n 6 --no-pager 2>/dev/null
        fi
        sleep 5
    done
}
...
log_analysis() {
    if [[ ! -f "$F2B_LOGFILE" ]]; then
        print_error "日志文件不存在"; return
    fi
...
fix_boot_order() {
    local svcs
    svcs=($(get_active_firewall_services))
...
main_menu() {
    while true; do
        ...
        case "$opt" in
           ...
           13) echo "当前后端: $FIREWALL_BACKEND"
               echo "可选: ufw / nftables-multiport / iptables-multiport"
               read -rp "  输入: " newba
               if [[ -n "$newba" ]]; then
                   INI_SET /etc/fail2ban/jail.local DEFAULT banaction "$newba"
                   restart_and_diagnose
               fi; pause ;;
