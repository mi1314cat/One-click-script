...
install_nodepass() {
    local force="${1:-false}"
    if [[ -x "$BIN_PATH" && "$force" != "true" ]]; then return; fi
...
# ================================
generate_tunnel_key() {
    openssl rand -base64 32   # 44 字符，足够安全
}
...
add_tunnel() {
    ...
    if [[ "$protocol" != "tcp" ]]; then
        path=$(safe_read "隧道路径（以 / 开头，至少 16 字符）" "/")
        while [[ ${#path} -lt 16 ]]; do
