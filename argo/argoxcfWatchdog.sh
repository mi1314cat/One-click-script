...
check_all_ports() {
    local ok=0; local ports; ports=$(get_gost_ports)
    [[ -z "$ports" ]] && { log "NO GOST PORTS"; return 1; }
    for p in $ports; do check_port "$p" && ok=1; done
    if [[ $ok -eq 1 ]]; then log "PORT CHECK OK"; return 0; else log "PORT FAIL"; return 1; fi
}
        ...
        if check_all_ports; then
            ...
            if (( FAIL_COUNT >= FAIL_THRESHOLD )); then
                if (( CONSECUTIVE_FAILURES >= MAX_RESTART_ATTEMPTS )); then
                    log "MAX ATTEMPTS REACHED, STOPPING"; sleep 600; continue
                fi
...
stop_watchdog() {
    ...
    # 额外清理可能的残留（通过进程名）
    pkill -f "bash.*${SCRIPT_NAME} run" 2>/dev/null || true
    sleep 0.5
    echo "✅ 已停止所有 watchdog"
}
...
start_watchdog() {
    ...
    if kill -0 $new_pid 2>/dev/null; then
        echo "✅ watchdog 已启动 (PID $new_pid)"
    else
        echo "❌ 启动失败，请检查日志"
    fi
}

restart_watchdog() { stop_watchdog; sleep 1; start_watchdog; }
...
get_next_restart_time() {
    ...
    if [[ "${BACKOFF_STAGE:-0}" -gt 0 && "${BACKOFF_START_TIME:-0}" -gt 0 ]]; then
        local end=$(( BACKOFF_START_TIME + BACKOFF_DELAY ))
        local now=$(date +%s)
        (( end > now )) && { date -d "@$end" '+%F %T' 2>/dev/null || echo "约 $((end-now)) 秒后"; return; }
    fi
    echo "无"
}
...
config_panel() {
    while true; do
        ...
        read -rp "请选择: " num
        case "$num" in
            1) read -rp "新值: " v; [[ -n "$v" ]] && { CHECK_INTERVAL="$v"; save_config; echo "已设为 ${CHECK_INTERVAL}"; }; sleep 0.3 ;;
            2) read -rp "新值: " v; [[ -n "$v" ]] && { FAIL_THRESHOLD="$v"; save_config; echo "已设为 ${FAIL_THRESHOLD}"; }; sleep 0.3 ;;
            3) read -rp "新值: " v; [[ -n "$v" ]] && { TCP_TIMEOUT="$v"; save_config; echo "已设为 ${TCP_TIMEOUT}"; }; sleep 0.3 ;;
            4) read -rp "新值: " v; [[ -n "$v" ]] && { CFD_RESTART_DELAY="$v"; save_config; echo "已设为 ${CFD_RESTART_DELAY}"; }; sleep 0.3 ;;
            5) read -rp "新值: " v; [[ -n "$v" ]] && { ARGO_RESTART_DELAY="$v"; save_config; echo "已设为 ${ARGO_RESTART_DELAY}"; }; sleep 0.3 ;;
            6) read -rp "新值: " v; [[ -n "$v" ]] && { RESTART_CHECK_DELAY="$v"; save_config; echo "已设为 ${RESTART_CHECK_DELAY}"; }; sleep 0.3 ;;
            7) read -rp "新值: " v; [[ -n "$v" ]] && { FIRST_BACKOFF="$v"; save_config; echo "已设为 ${FIRST_BACKOFF}"; }; sleep 0.3 ;;
            8) read -rp "新值: " v; [[ -n "$v" ]] && { SECOND_BACKOFF="$v"; save_config; echo "已设为 ${SECOND_BACKOFF}"; }; sleep 0.3 ;;
            9) read -rp "新值: " v; [[ -n "$v" ]] && { RECOVERY_COOLDOWN="$v"; save_config; echo "已设为 ${RECOVERY_COOLDOWN}"; }; sleep 0.3 ;;
            10) read -rp "新值: " v; [[ -n "$v" ]] && { MAX_RESTART_ATTEMPTS="$v"; save_config; echo "已设为 ${MAX_RESTART_ATTEMPTS}"; }; sleep 0.3 ;;
            11) restart_watchdog ;;
            0) break ;;
            *) echo "无效选项"; sleep 0.3 ;;
        esac
...
delete_watchdog() {
    stop_watchdog
    rm -rf "$BASE_DIR" "$PID_FILE" "$LOCK_FILE" /etc/systemd/system/cfd-watchdog.service
...
if [[ "${1:-}" == "run" ]]; then
    ...
    exec 9>"$LOCK_FILE"
    if ! flock -n 9; then
        echo "[$(date '+%F %T')] watchdog already running via lock, exiting"
        exit 0
    fi
    # 写入 PID
    echo $$ > "$PID_FILE"
